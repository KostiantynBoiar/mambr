...
\begin{frame}{The problem}
  \begin{itemize}
    ...
    \item On the \textbf{MammAlps} benchmark:
      \begin{itemize}
        \item audio gives a small lift over video (0.453 $\to$ 0.473 class-avg mAP);
        \item \textbf{scene segmentation \emph{degraded} it} --- video fell 0.453 $\to$
              0.403, and video+audio+seg (0.466) stayed below video+audio (0.473).
      \end{itemize}
  \end{itemize}
  \vfill
  \begin{block}{The gap}
...
\begin{frame}{The idea (and the novelty)}
  ...
  \begin{itemize}
    \item Extract it with \textbf{SAM\,2}, tracked across the clip; fuse it on top of
          video\,+\,audio.
    \item A silhouette's changing shape \emph{is} posture --- an explicit, background-free
          cue the encoders only see entangled with appearance.
  \end{itemize}
  \vfill
  \textbf{What is actually new} --- shape itself is \emph{not} (it is long established in
  human action \& gait recognition; Bobick \& Davis 2001, Gorelick et al.\ 2007):
  \begin{itemize}
    \item we \textbf{adapt} silhouette-shape to \emph{multimodal wildlife} behaviour
          recognition --- as a fused, \emph{positive} modality, not a background-removal tool.
  \end{itemize}
  \vfill
...
\begin{frame}{Closest work: same mask, \emph{opposite} purpose}
  ...
  \begin{tabular}{@{}lll@{}}
    ...
    Fed to the model & RGB \emph{pixels} (appearance) & \emph{geometry} (outline, area) \\
...
\begin{frame}{Plan / next steps}
  \begin{enumerate}
    ...
    \item Reproduce the \textbf{video} and \textbf{video+audio} baseline
          (target 0.453 / 0.473 class-avg mAP --- the \emph{MammAlps paper's reported
          scores}, which we aim to match).
  ...
  \end{enumerate}
  \vfill
  \begin{block}{Pre-registered \& honest}
    Ablations fixed up front --- shape-only / video+shape / full fusion; $\geq 5$ seeds with
    \textbf{95\,\% CIs}; the success threshold ($+0.02$ mAP, or $+0.03$ per-class on contact
    behaviours) decided \emph{before} running. Even a clean negative is publishable.
  \end{block}
\end{frame}
...
\begin{frame}{Discussion}
  \begin{itemize}
    \item Does the scope (single-animal MammAlps clips; shape-as-a-modality with a
          background-robustness test) look right?
