...
Pose-based pipelines (e.g. ST-GCN~\cite{stgcn} and PoseR~\cite{poser}) instead encode the animal's skeleton. Pose is compact and effective, but keypoints capture body \emph{configuration} only sparsely---a handful of landmarks, not the full outline that \emph{defines} posture. LookAgain~\cite{lookagain} reports a related asymmetry: added visual context helps some behaviours yet not those whose geometry pose already pins down.

The gap is the obvious next question nobody has tested. If the background does not help (MammAlps) and models over-rely on it (PanAf-FGBG), what about the one representation that throws the background away entirely and keeps only the animal's shape? This is a different \emph{object} of segmentation: where MammAlps' map is a \emph{static, scene-level} segmentation of the background, ours is a \emph{per-frame foreground} (instance) segmentation of the animal itself---a silhouette whose changing outline \emph{is} posture. Treating this \emph{foreground silhouette as a fused, positive modality}---rather than a background-removal tool or an ROI crop---is unaddressed.
...
\begin{description}[leftmargin=2.2em,style=nextline]
  ...
  \item[RQ3 (robustness)] Is a shape-based model more robust across \emph{unseen camera locations} (background shift) than background-dependent video features---a direct extension of PanAf-FGBG's finding?
\end{description}

\noindent\textbf{Hypothesis.} Foreground shape carries posture information complementary to pose and video. Fusing it will (i) match or exceed the audio-sized gain on the aggregate metric, (ii) show its clearest per-behaviour gains on contact/posture classes, and (iii) degrade more gracefully than video-only features under background shift.

% ---------------------------------------------------------------------
\section{Proposed approach}
% ---------------------------------------------------------------------
\paragraph{Dataset.} \emph{MammAlps}~\cite{mammalps} is the dataset: it natively carries video, audio and behaviour labels, and supplies the published baseline to beat. Its single-animal clips across five species are the setting for this shape-as-a-modality study. (The complementary multi-animal question---\emph{inter-animal} mask-contact geometry on MABe22~\cite{mabe22}---is pursued as a \emph{separate} track and is deliberately out of scope here.)

\paragraph{Architecture.} Figure~\ref{fig:arch} shows the design. The heavy models are \textbf{off-the-shelf and pretrained}---we run them as-is and \emph{never update their weights}---so their outputs are computed once, offline, and cached to disk (Table~\ref{tab:frozen}). VideoMAE for the clip and an AST/AudioMAE-style audio model over log-mel spectrograms are \emph{encoders}: each turns a clip into a fixed vector. SAM\,2~\cite{sam2} is different---it is a \emph{segmentation} model, prompted by the dataset's animal tracks/boxes and propagated across the clip, that outputs a per-frame \emph{mask} rather than an embedding. The silhouette is reduced to cheap, interpretable shape descriptors (area, aspect ratio, solidity, Hu moments), with a heavier frozen-DINOv2~\cite{dinov2} masked-crop embedding held in reserve only if the hand-crafted features underperform. The \emph{only} trainable component is a small fusion head: a tiny temporal encoder for the shape series, a one- or two-layer transformer (or even an MLP / late fusion) over the per-modality tokens, and ActionMAE-style modality dropout~\cite{actionmae} so the mask can be dropped at test time.
...
Shape & SAM\,2~\cite{sam2} $+$ shape descriptors & frame $+$ box $\rightarrow$ \emph{mask} $\rightarrow$ shape numbers \\
\bottomrule
\end{tabular}
\caption{The frozen building blocks: off-the-shelf pretrained models, run once and cached, \emph{not} trained by us. VideoMAE and AST are encoders (clip\,$\rightarrow$\,vector); SAM\,2 is a \emph{segmentation} model whose output mask feeds the shape descriptors. Only the small fusion head (plus the tiny shape temporal encoder) is trained.}
\label{tab:frozen}
\end{table}
...
\node[feat] (shp)  at (6.4,-2.6)  {Silhouette shape features\\[1pt]\scriptsize area, aspect ratio,\\ solidity, Hu moments};
% trainable fusion
...
% output
\node[result] (out) at (13.3, 0.0) {Behaviour\\[1pt]\scriptsize activity\\$+$ action};
% arrows
...
\paragraph{Training and protocol.} Only the head trains, on cached features, on an M4 Pro (PyTorch MPS). Use official splits with a fixed seed; report \emph{mean $\pm$ std over several seeds}, since on datasets this small a $+0.02$ mAP can be noise. Keep the head capacity identical across conditions so any gain reflects \emph{information}, not parameters; an optional control feeds a blanked/scrambled mask (same parameters, no real shape) to confirm the silhouette is doing the work. Hierarchical heads (activity + action) on MammAlps; balanced sampling and per-class reporting throughout.

% ---------------------------------------------------------------------
\section{Novelty}
% ---------------------------------------------------------------------
The contribution is \emph{not} a new shape representation: silhouette- and shape-based motion descriptors are long established in human action recognition (motion-energy/history templates~\cite{bobickdavis}; space-time shapes~\cite{spacetimeshapes}) and in gait recognition~\cite{gaitsurvey}. Our contribution is to \emph{adapt} silhouette-shape representations to \emph{multimodal wildlife} behaviour recognition---treating the foreground silhouette as a fused, positive modality rather than a background-removal tool---where it is largely unexplored. Pre-empting the obvious objection, each near neighbour is distinct:
\begin{itemize}
  ...
  \item \textbf{PanAf-FGBG}~\cite{panaf} (the closest work): the \emph{same tool} (SAM\,2 masks) used for the \emph{opposite} purpose. There the mask \emph{removes or swaps} the background, the object of study is the background itself (how much models over-rely on it), and the foreground---when kept---is still RGB \emph{appearance}. We instead keep the mask's \emph{geometry}, the silhouette \emph{shape}, as a positive input, and treat their robustness finding as motivation (shape discards the very background they show models lean on). Table~\ref{tab:vspanaf} makes the contrast explicit.
  ...
  \item \textbf{LookAgain}~\cite{lookagain}: keypoint$\leftrightarrow$video fusion using raw video, no masks; its per-behaviour asymmetry motivates a dedicated shape stream for posture behaviours.
\end{itemize}
...
\begin{tabular}{@{}lll@{}}
\toprule
 & \textbf{PanAf-FGBG}~\cite{panaf} (closest) & \textbf{This project} \\
\midrule
Mask is used to  & separate / erase / swap the \emph{background} & derive the \emph{silhouette shape} \\
Object of study  & the background (bias \& robustness)           & the foreground shape (a feature) \\
Fed to the model & RGB \emph{pixels} (FG/BG appearance)          & \emph{geometry} (outline, area) \\
Core question    & do models over-rely on background?            & does shape \emph{add} behaviour signal? \\
\bottomrule
\end{tabular}
\caption{The closest work uses the \emph{same} SAM\,2 masks for the \emph{opposite} purpose: it studies the background it removes, whereas we encode the foreground \emph{shape} it would otherwise discard.}
\label{tab:vspanaf}
\end{table}
...
\midrule
Video only                          & $0.453^{\dagger}$ & baseline to reproduce \\
\quad $+$ \emph{their} segmentation  & $0.403^{\dagger}$ & background maps \emph{degrade} video (our motivation) \\
\quad $+$ audio                      & $0.473^{\dagger}$ & audio adds $+0.02$ (the bar) \\
\quad $+$ mask (\emph{shape})        & \textbf{target $\gtrsim$ audio} & RQ1/RQ2: does \emph{our} shape match/beat the audio gain? \\
Video $+$ audio $+$ mask             & \textbf{?} & RQ1: does shape add \emph{on top of} audio? \\
\bottomrule
\end{tabular}
\caption{Anticipated pattern (not measured results). $^{\dagger}$Figures reported by MammAlps~\cite{mammalps} (Table~4); their video+audio+segmentation scored $0.466$. Segmentation \emph{lowered} video ($0.453\to0.403$), motivating a foreground-\emph{shape} stream rather than a scene map. Baselines to be reproduced from cached features before any comparison is claimed.}
\label{tab:expected}
\end{table}

\noindent\textbf{What each outcome means.}
\begin{itemize}
  \item \emph{Strong positive:} video+mask beats video-only by at least the audio gain, \emph{or} a clear per-class win ($\sim+0.03$--$0.05$ AP) on contact/posture behaviours (e.g.\ chasing, mounting). Headline: shape helps where background maps did not, and for the ``right'' (background-free) reasons---supported by the robustness test (RQ3).
  \item \emph{Publishable either way:} any consistent per-class gain on contact/posture classes, with mask-dropout robustness demonstrated.
...
\paragraph{Pre-registered ablations.} To keep a $+0.02$-type result interpretable against seed noise, the comparison set \emph{and} the success criterion are fixed \emph{before} running. We isolate: (a)~\textbf{shape descriptors alone} (does shape carry any signal at all?); (b)~\textbf{video vs.\ video+shape} (the key isolation); and (c)~\textbf{full fusion}, video+audio+shape vs.\ video+audio (does shape add \emph{on top of} audio?). Each condition runs over $\geq 5$ seeds with class-balanced sampling and \emph{identical} head capacity, reported as mean class-average mAP with \textbf{95\,\% confidence intervals}, plus per-class AP on the contact/posture classes. \textbf{Success is decided in advance:} shape helps iff (i)~video+shape exceeds video with a 95\,\% CI excluding zero \emph{and} a pre-registered minimum effect of $+0.02$ mAP (the audio-sized gain), \emph{or} (ii)~a per-class AP gain $\geq +0.03$ on contact/posture behaviours (chasing, courtship). A blanked/scrambled-mask control (same parameters, no real shape) rules out capacity effects.

% ---------------------------------------------------------------------
\section{Feasibility}
% ---------------------------------------------------------------------
None of the heavy models is ever in the training loop: each is run once on Kaggle's free GPU and its outputs cached, so they are a bounded, one-time preprocessing cost rather than a per-step one. The trained head sees only a handful of cached vectors, so it is tiny and re-runs in minutes on a laptop---keeping the experiment in line with PoseR's lightness philosophy, whose real target is cheap \emph{deployment} over long footage. If shape proves useful and cheap inference later matters, there are ready routes (keep the hand-crafted branch, drop the mask via the modality-dropout design, or distil), treated here as future work. The one genuinely heavy step is the single SAM pass to generate masks; this is batched, cached, and gated by a checkpoint around week~5 to reassess if MammAlps night/infrared footage makes masking too slow or noisy---at which point the shape stream falls back to the daytime clips where the silhouette is reliable.
...
\textbf{Wk 2} & SAM\,2 on Kaggle; generate + cache silhouette masks for MammAlps.\\
\textbf{Wk 3} & Silhouette shape-feature extractor (area, aspect ratio, solidity, Hu moments); sanity checks.\\
\textbf{Wk 4} & Light fusion head + modality dropout; video vs.\ video+mask, per behaviour.\\
\textbf{Wk 5} & QC mask quality (incl.\ night/infrared footage); go/no-go decision.\\
